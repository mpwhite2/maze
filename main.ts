scene.onOverlapTile(SpriteKind.Player, assets.tile`myTile0`, function (sprite, location) {
    info.changeScoreBy(20)
    game.over(true)
})
controller.B.onEvent(ControllerButtonEvent.Pressed, function () {
    Render.toggleViewMode()
})
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    Render.jumpWithHeightAndDuration(Player1, 35, 1000)
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.Projectile, function (sprite, otherSprite) {
    otherSprite.destroy(effects.confetti, 500)
    info.changeLifeBy(1)
})
spriteutils.onSpriteKindUpdateInterval(SpriteKind.Enemy, 500, function (sprite) {
    if (spriteutils.getSpritesWithin(SpriteKind.Player, 60, sprite).length > 0 && Math.percentChance(15)) {
        Render.jumpWithHeightAndDuration(sprite, 22, 1000)
    }
})
info.onLifeZero(function () {
    game.over(false)
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.Food, function (sprite, otherSprite) {
    otherSprite.destroy()
    info.changeScoreBy(1)
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function (sprite, otherSprite) {
    if (Render.isSpritesOverlapZ(sprite, otherSprite)) {
        if (Render.getSpriteAttribute(sprite, RCSpriteAttribute.ZVelocity) < 0) {
            music.play(music.melodyPlayable(music.jumpUp), music.PlaybackMode.InBackground)
            info.changeScoreBy(1)
            sprites.destroy(otherSprite)
        } else {
            info.changeLifeBy(-1)
            otherSprite.destroy(effects.blizzard, 500)
            sprite.startEffect(effects.ashes, 500)
            music.wawawawaa.play()
            scene.cameraShake(8, 500)
            pause(1000)
        }
    }
})
let Snack: Sprite = null
let Chest: Sprite = null
let Enemy1: Sprite = null
let Player1: Sprite = null
scene.setBackgroundImage(img`
    
    `)
tiles.setCurrentTilemap(tilemap`level1`)
Player1 = Render.getRenderSpriteInstance()
Player1.x += 6
Render.moveWithController(3)
info.setLife(3)
info.setScore(0)
for (let index = 0; index < 20; index++) {
    Enemy1 = sprites.create(img`
        ........................
        ........................
        ........................
        ........................
        ..........ffff..........
        ........ff1111ff........
        .......fb111111bf.......
        .......f11111111f.......
        ......fd11111111df......
        ......fd11111111df......
        ......fddd1111dddf......
        ......fbdbfddfbdbf......
        ......fcdcf11fcdcf......
        .......fb111111bf.......
        ......fffcdb1bdffff.....
        ....fc111cbfbfc111cf....
        ....f1b1b1ffff1b1b1f....
        ....fbfbffffffbfbfbf....
        .........ffffff.........
        ...........fff..........
        ........................
        ........................
        ........................
        ........................
        `, SpriteKind.Enemy)
    tiles.placeOnRandomTile(Enemy1, assets.tile`transparency16`)
    Enemy1.follow(Player1, 40)
}
for (let index = 0; index < 2; index++) {
    Chest = sprites.create(img`
        . . b b b b b b b b b b b b . . 
        . b e 4 4 4 4 4 4 4 4 4 4 e b . 
        b e 4 4 4 4 4 4 4 4 4 4 4 4 e b 
        b e 4 4 4 4 4 4 4 4 4 4 4 4 e b 
        b e 4 4 4 4 4 4 4 4 4 4 4 4 e b 
        b e e 4 4 4 4 4 4 4 4 4 4 e e b 
        b e e e e e e e e e e e e e e b 
        b e e e e e e e e e e e e e e b 
        b b b b b b b d d b b b b b b b 
        c b b b b b b c c b b b b b b c 
        c c c c c c b c c b c c c c c c 
        b e e e e e c b b c e e e e e b 
        b e e e e e e e e e e e e e e b 
        b c e e e e e e e e e e e e c b 
        b b b b b b b b b b b b b b b b 
        . b b . . . . . . . . . . b b . 
        `, SpriteKind.Projectile)
    tiles.placeOnRandomTile(Chest, assets.tile`transparency16`)
}
for (let index = 0; index < 100; index++) {
    Snack = sprites.create(img`
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . 5 5 . . . . . . . 
        . . . . . . . 5 5 . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        `, SpriteKind.Food)
    tiles.placeOnRandomTile(Snack, assets.tile`transparency16`)
}
let Enemy2 = sprites.create(img`
    ........................
    ........................
    ........................
    ........................
    ........................
    ..........ffff..........
    ........ff1111ff........
    .......fb111111bf.......
    .....fffc1111111f.......
    ...fc111cd1111111f......
    ...f1b1b1b1111dddf......
    ...fbfbffcf11fcddf......
    ......fcf111111bbf......
    .......ccbdb1b1fcf......
    .......fffbfbfdff.......
    ........ffffffff........
    ........fffffffffff.....
    .........fffffc111cf....
    .........fffff1b1b1f....
    ..........ffffbfbfbf....
    ...........ffff.........
    ........................
    ........................
    ........................
    `, SpriteKind.Enemy)
Enemy2.setFlag(SpriteFlag.GhostThroughWalls, false)
tiles.placeOnRandomTile(Enemy2, assets.tile`transparency16`)
Enemy2.follow(Player1, 50)
forever(function () {
    music.playMelody("C5 A B G A F G E ", 120)
    if (info.score() > 19) {
        tiles.setWallAt(tiles.getTileLocation(1, 1), false)
        tiles.setTileAt(tiles.getTileLocation(1, 1), assets.tile`transparency16`)
        tiles.setTileAt(tiles.getTileLocation(1, 1), assets.tile`myTile0`)
    }
})
